...
# Executable in ONNX Runtime today: the FP32 cache at any retention, and the
# int8 per-head cache through the integer-attention graph (built for medium,
# E2). int4 and the KIVI grouping are NumPy simulations: capacity headroom,
# not candidates for the operating point.
EXECUTABLE = {"medium_uz": ("FP32", "int8 head"), "small_en": ("FP32",)}


def executable(name, lab):
    return any(lab.startswith(e) for e in EXECUTABLE[name])


def select(name, C, ref, alpha, m_l3=M_L3, only_executable=True):
    L, w = LAYERS[name], W_LAYER[name]
    delta = round(ref * EPS, 4)
    b_kv = alpha * m_l3 - w
    pool = [c for c in C if executable(name, c[0])] if only_executable else C
    fits = [(mib, lab) for lab, mib, d, _ in pool if mib / L <= b_kv and round(d[2], 4) < delta]
...
def main():
    ...
    for name in ("medium_uz", "small_en"):
        ...
        for a in (0.5, 0.6, 0.7, 0.8):
            ...
            _, hw_any, _ = select(name, C, ref, a, only_executable=False)
            res[name]["alpha"][str(a)] = {"budget_kv_layer": b_kv, "hardware_point": hw,
                                          "hardware_point_any": hw_any, "max_gated": mx}
            print(f"  alpha {a}: B_KV/layer {b_kv:5.2f} MiB -> executable: "
                  f"{hw[1] + f' ({hw[0]:.1f} MiB)' if hw else 'NONE fits'}"
                  f" | any measured: {hw_any[1] + f' ({hw_any[0]:.1f} MiB)' if hw_any else 'NONE'}"
        ...
        for m in sorted({8, 12, 16, 24, 32, 48, M_L3}):
            ...
            _, hw_any, _ = select(name, C, ref, 0.7, m, only_executable=False)
            res[name]["l3"][str(m)] = {"budget_kv_layer": b_kv, "hardware_point": hw,
                                       "hardware_point_any": hw_any, "max_gated": mx}
            print(f"  L3 {m:4.1f} MiB (alpha 0.7): B_KV/layer {b_kv:6.2f} -> executable: "
                  f"{hw[1] + f' ({hw[0]:.1f} MiB)' if hw else 'NONE fits'}"
                  f" | any measured: {hw_any[1] + f' ({hw_any[0]:.1f} MiB)' if hw_any else 'NONE'}")
